            ...
            using (var ctx = new ProjectContext())
            {
                switch (cmbOverzicht.SelectedIndex)
                {
                    case 0:
                        ...
                        break;
                    case 1:
                        var productsor = ctx.OrderDetails.GroupBy(od => od.product).Select(od => new { product = od.Key.ProductNaam + " [" + od.Key.ProductNummer + "]", totaal = od.Count() }).OrderByDescending(od=>od.totaal).ToList();
                        ...
                        break;
                    case 2:
                        var products1 = ctx.OrderDetails.GroupBy(od => od.product).OrderBy(od => od.Sum(ods => ods.Aantal)).Select(od => new { product = od.Key.ProductNaam + " [" + od.Key.ProductNummer + "]", totaal = od.Sum(ods => ods.Aantal) }).ToList();
                        ...
                        break;
                    case 3:
                        var products1or = ctx.OrderDetails.GroupBy(od => od.product).Select(od => new { product = od.Key.ProductNaam + " [" + od.Key.ProductNummer + "]", totaal = od.Count() }).OrderBy(od => od.totaal).ToList();
                        ...
                        break;
                    case 4:
                        var klants = ctx.Klanten.Join(ctx.Orders.Join(ctx.OrderDetails, o => o.OrderId, od => od.order.OrderId, (o, od) => new { o.OrderId, o.klant.KlantId, totaal = od.Aantal * od.product.UnitPrice }), k => k.KlantId, ood => ood.KlantId, (k, ood) => new { klant = k.Voornaam + " " + k.achternaam, totaal = ood.totaal }).OrderByDescending(kood => kood.totaal).GroupBy(kood => kood.klant).Select(kood => new { klant = kood.Key, totaal = kood.Sum(koods => koods.totaal) }).ToList();
                        ...
                        break;
                    case 5:
                        var klantsor = ctx.Orders.GroupBy(o=>o.klant).Select(o => new {klantor= o.Key.Voornaam+" "+o.Key.achternaam , totaal=o.Count() }).ToList();
                        ...
                        break;
                    case 6:
                        var klants1 = ctx.Users.Join(ctx.Orders.Join(ctx.OrderDetails, o => o.OrderId, od => od.order.OrderId, (o, od) => new { o.OrderId, o.user.UserId, totaal = od.Aantal * od.product.UnitPrice }), k => k.UserId, ood => ood.UserId, (k, ood) => new { verkoper = k.Voornaam + " " + k.Achternaam, totaal = ood.totaal }).OrderByDescending(kood => kood.totaal).GroupBy(kood => kood.verkoper).Select(kood => new { verkoper = kood.Key, totaal = kood.Sum(koods => koods.totaal) }).ToList();
                    ...
                    case 7:
                        ...
                        break;
                    case 8:
                        var bezorgers = ctx.Bezorgers.Join(ctx.Orders, b => b.BezorgerId, o => o.BezorgdDoor.BezorgerId, (b, o) => new { b, o }).Where(bo=>bo.o.status==OrderStatus.Verzonden).GroupBy(bo => bo.b).Select(bo => new { bezorger = bo.Key.Naam, totaal = bo.Count() }).ToList();
                        ...
                        break;
                    case 9:
                        var incomes = ctx.OrderDetails.GroupBy(bo => bo.order.OrderDatum.Day).Select(bo => new { date = bo.Key, incomes = bo.Select(b=>b.Aantal*b.product.UnitPrice).Sum() }).ToList();
                        ...
                        break;
                    case 10:
                        var orders = ctx.Orders.GroupBy(bo => bo.OrderDatum.Day).Select(bo => new { date = bo.Key, orders = bo.Count()}).ToList();
                        ...
                        break;
                    case 11:
                        var klanten = ctx.Klanten.GroupBy(bo => bo.IngevoegdDatum.Day).Select(bo => new { date = bo.Key, klanten = bo.Count() }).ToList();
